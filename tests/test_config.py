...
from agentlink.config import Config, find_config_path, load_config
...
def test_find_config_path(tmp_path: Path, monkeypatch):
    ...
    (tmp_path / ".agentlink.yaml").write_text("source: a\nlinks: [b]\n", encoding="utf-8")
    path, is_project = find_config_path()
    assert is_project
    assert path == (tmp_path / ".agentlink.yaml").resolve()
