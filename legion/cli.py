...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="legion", description="Keep your AI instruction files in sync with zero magic — just symlinks")
    ...
    subs.add_parser("init", help="Create .legion.yaml in current directory")
...
def run_init(args: argparse.Namespace) -> int:
    config_path = Path(".legion.yaml")
    if config_path.exists() and not args.force:
        print_error(".legion.yaml already exists (use --force to overwrite)")
        return 1

    if not Path(".git").exists() and not args.force:
        response = input("No .git directory found. Create .legion.yaml here anyway? (y/N): ").strip().lower()
        if response not in {"y", "yes"}:
            print_info("Cancelled")
            return 0

    if args.dry_run:
        print_info("Would create .legion.yaml")
        return 0

    config.create_project_config(config_path)
    print_ok(f"Created {config_path.resolve()}")
    print_info("Edit the config file and run 'legion sync' after creating your source file")
    return 0
...
def load_or_create_config(config_path: Path, is_project: bool, dry_run: bool):
    ...
    if is_project:
        print_error("No .legion.yaml found in current directory")
        print_info("Run 'legion init' to create one")
...
def run_check(args: argparse.Namespace) -> int:
    ...
    if not config_path.exists():
        print_error("No .legion.yaml found in current directory" if is_project else f"No global config found at {config_path}")
        return 1
    ...
    if has_problems:
        print("\nFound problems. Run 'legion sync' to fix them.")
        return 1
...
def _check_symlink_support() -> None:
    ...
    target = tmp / "legion_test_target"
    link = tmp / "legion_test_link"
...
def run_doctor(args: argparse.Namespace) -> int:
    ...
    print(("✓" if shutil.which("legion") else "⚠️") + " Binary is{} in PATH".format("" if shutil.which("legion") else " not"))
    print()

    cfg_dir = Path.home() / ".config" / "legion"
    ...
    print("✓ Project config found: .legion.yaml" if Path(".legion.yaml").exists() else "⚠️  No project config (.legion.yaml)")
    print()
